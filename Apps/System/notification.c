...
static void _notif_window_unload(void);
...
static void _notif_window_load(Window *window)
{
    ...
    Notification *notification = notification_create(app, title, body, gbitmap_create_with_resource(RESOURCE_ID_ROUNDRECT_THING), GColorRed);
        SYS_LOG("noty", APP_LOG_LEVEL_INFO, "asas Create %d", _notif_layer);
    ...
    SYS_LOG("noty", APP_LOG_LEVEL_INFO, "asas Create %d", notification_layer_get_layer(_notif_layer));
